...
# cosine matrix #
a11=$(echo "$x1 $x2 $x3"| awk '{printf ("%.8f\n",($1*0+$2*1+$3*0)/sqrt(1)/sqrt((($1*$1+$2*$2+$3*$3))))}')
a12=$(echo "$x1 $x2 $x3"| awk '{printf ("%.8f\n",($1*(-2)+$2*1+$3*0)/sqrt(5)/sqrt((($1*$1+$2*$2+$3*$3))))}')
...
V=17.33925                 ##就用钛原胞体积除以2，即所谓atomic volume，以后也可能是超胞体积除以总原子数
G=51.91316
u=0.29949
b=2.93562
